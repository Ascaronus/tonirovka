...
async function run(fallback) {
    ...
    click(urls.telegram,'footer');assert.equal(sent.length,12);
    clock+=1001;click(urls.telegram,'footer','auxclick');assert.equal(sent.length,13);
    ...
    click(urls.facebook,'contacts','auxclick',{button:2});assert.equal(sent.length,13);
    clock+=1001;click(urls.facebook,'contacts','click',{button:0,detail:0});assert.equal(sent.length,14); // Enter activation
    await new Promise(r=>setImmediate(r));
}
(async()=>{for(const fallback of [false,true,'throw'])await run(fallback);console.log('Client: 4 services × 3 positions, repeats, keyboard/middle clicks, beacon/fetch failures and navigation passed');})().catch(e=>{console.error(e);process.exit(1);});
