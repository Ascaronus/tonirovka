...
(async()=>{
 ...
 for (const width of [320,375,390,768,1440]) {
  ...
  const phoneResponse=page.waitForResponse(r=>r.url().endsWith('/contact-click.php')&&r.request().postData()?.includes('service=phone'));
  await bar.locator('.contact-phone-action').click();
  assert.equal((await phoneResponse).status(),204,'Phone clicks recorded');
  ...
  await page.evaluate(()=>document.addEventListener('click',e=>{if(e.target.closest('footer a[href^="tel:"]'))e.preventDefault();}));
  const footerCall=page.waitForResponse(r=>r.url().endsWith('/contact-click.php')&&r.request().postData()?.includes('service=phone&place=footer'));
  await page.locator('footer .contact-phone-action').click();
  assert.equal((await footerCall).status(),204,'Footer phone clicks recorded');
