...
function seoApplyMeta($html,$meta) {
    ...
    foreach(['description'=>'description','og:title'=>'title','og:description'=>'description','twitter:title'=>'title','twitter:description'=>'description','DC.title'=>'title','DC.description'=>'description'] as $name=>$key){$attribute=str_contains($name,':')?'property':'name';$tag='<meta '.$attribute.'="'.$name.'" data-lang-uk="'.$esc($meta['uk'][$key]).'" data-lang-ru="'.$esc($meta['ru'][$key]).'" content="'.$esc($meta['uk'][$key]).'">';$pattern='~<meta\b(?=[^>]*(?:name|property)="'.preg_quote($name,'~').'" )[^>]*>~s';
